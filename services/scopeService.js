...
export const processScopeGeneration = async (userId, questionnaireId) => {
  ...
  const mcqParsed =
    typeof questionnaire.mcq_answers === "string"
      ? JSON.parse(questionnaire.mcq_answers)
      : questionnaire.mcq_answers;

  return await callLLM(
    questionnaire.project_overview,
    mcqParsed?.purpose || "General Application",
  );
};

export const processScopeRegeneration = async (
  userId,
  questionnaireId,
  feedbackText,
) => {
...
export const fetchScopeForUser = async (userId, scopeId) => {
  ...
  rows[0].scope_text =
    typeof rows[0].scope_text === "string"
      ? JSON.parse(rows[0].scope_text)
      : rows[0].scope_text;
  return rows[0];
};
...
export const assignDeveloperToProject = async (
    projectId,
    developerId
) => {
